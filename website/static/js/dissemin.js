    ...
    $("input[type='radio'][name='radioRepository']").click(function(){
        ...
        $.ajax({
            ...
            success : function(data) {
                ...
                $('.select2').select2();
                // Sometimes the dropdown is misplaced, this has something to do with where the dropwon is placed.
                // However, Select2 is aware of the issue, there's not yet a soluton.
                // What works is to recalculate the position on usage.
                // See https://api.archives-ouvertes.fr/sword/
                var select2Data = $('.select2').data('select2')
                if (select2Data) {
                    select2Data.on('results:message', function(params){
